...
| 0013 | `0013_agents_hiring.sql` | **agent hiring parity**: `agents` (employees) + `approvals` + company toggle `require_board_approval_for_new_agents`; `hire_agent`/`decide_approval` (+ `_with_key`); a hired agent does jobs attributed to it (`jobs.subject_type='agent'`). Governance: **pending_approval limbo** (can't mint key / run), **board-only approve** (on the human minter's role), **agents may never decide**, `can_create_agents` enforced, terminal/idempotent decide. Supersedes `create_api_key`/`create_job_with_key` with agent-active guards. |
